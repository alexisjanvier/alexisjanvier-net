...
Mon environnement de travail a beaucoup évolué au cours de ma carrière. De mes débuts en PHP avec [Eclipse](https://www.eclipse.org/ide/), à [Sublime Text](https://www.sublimetext.com/) quand je suis passé sous mac, puis [Atom](https://atom.io/) ou encore [Visual Studio Code](https://code.visualstudio.com/) sous Linux, j'ai pas mal pratiqué les éditeurs GUI (pour graphical user interface). 

En parallèle, j'ai dû petit à petit apprendre à utiliser la console, que ce soit pour utiliser des outils uniquement disponibles en cli (command-line interface), du genre [Subversion](https://subversion.apache.org/), [Git](https://git-scm.com/) au début ou encore [gulp.js](https://gulpjs.com/), mais aussi pour gérer des serveurs.

C'est pourtant il n'y a pas si longtemps, avec l'adoption de [vim](https://www.vim.org/)/[Neovim](https://neovim.io/) comme éditeur de code, que la console est véritablement devenue mon environnement de développement. Et pour filer la métaphore [Software Craftsmanship](https://manifesto.softwarecraftsmanship.org/#/fr-fr), l'artisanat du logiciel, j'aime bien appréhender cette console comme mon atelier de travail.

Mais avant d'aller plus loin, de quoi parle-t-on quand on parle de la console ?
...
![](6_drawings/shell.excalidraw.png)
...
Il existe plusieurs shells, les plus courants étant **sh** (appelé Bourne shell),  
**bash** (Bourne again shell), **csh** (C Shell), **Tcsh** (Tenex C shell), **ksh** (Korn shell) et **zsh** (Zero shell). 
...
![](3_travaux/1_travaux-en-cours/la-console/attachments/terminal.jpeg)
...
![](2_craftsmanship/journey/attachments/archlinux.png)

### Les entrées-sorties standard

L'exécution d'une commande au sein d'une console va créer un **processus**, qui ouvrir trois flux :

0 -   **stdin*** permettant au processus de lire les données d'entrée. Par défaut _stdin_ correspond au clavier.
1 -   **stdout** permettant au processus d'écrire les données de sortie. Par défaut _stdin_ correspond à l'écran.
2 -   **stderr**permettant au processus d'envoyer un message en cas d'erreur. Par défaut _stderr_ correspond à l'écran.

![](6_drawings/std-in-out-error.excalidraw.png)
...
de l'entrée standard. La commande suivante envoie le contenu du fichier mesprojets.txt en entrée de la commande `cat`, dont le seul but est d'afficher le contenu sur la sortie standard :  
...
Par exemple, vous pouvez utiliser ensemble les cli `history`, `grep`, `sort`, `uniq` et `head` pour afficher les cli que vous utilisez le plus et la fréquence de leur utilisation.
...
Passé cette un peu longue, mais je l'espère instructive et contextualisant introduction, je vais maintenant enchainer sur ce qui fait que ma console est mon atelier de travail, façonné par mes préférences et mes habitudes. J'y parlerais du mon choix de shell et d'émulateur de terminal, mais aussi des cli que j'apprécie le plus. Ce ne sera pas forcément votre atelier idéal, mais je souhaite qu'il puisse vous servir d'inspiration ou vous faire découvrir quelques bons outils.

Je vais donc découper cette suite en 6 posts :

- [Un atelier ouvert et transposable grâce aux dotfiles](3_travaux/1_travaux-en-cours/la-console/02-dotfiles.md)
- [Pourquoi j'ai choisi le shell zsh](3_travaux/1_travaux-en-cours/la-console/03-shell.md)
- [L'émulateur de terminal kitty](3_travaux/1_travaux-en-cours/la-console/04-kitty.md)
- [Mes commandes du quotidien](3_travaux/1_travaux-en-cours/la-console/05-daily-clis.md)
- [Mes commandes git](3_travaux/1_travaux-en-cours/la-console/06-git.md)
